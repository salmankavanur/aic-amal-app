...
export default function HafizSponsorshipPage() {
  ...
  useEffect(() => {
    applyFilters();
  }, [searchText, selectedPeriod, dateFrom, dateTo, sponsorships]);
  ...
  const calculateStats = () => {
    const monthlySponsors = sponsorships.filter(s => s.period === "1 Month").length;
    const quarterlySponsors = sponsorships.filter(s => s.period === "6 Months").length;
    const yearlySponsors = sponsorships.filter(s => s.period === "One Year" || "One Year(with education)").length;
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="bg-transparent md:bg-white/10 backdrop-blur-md rounded-2xl p-4 md:p-6 border border-none md:border-white/20 shadow-xl">
        ...
        <SponsorshipTable
          ...
          sortBy={sortBy}
          sortOrder={sortOrder} onViewDetails={function (sponsorship: Sponsorship): void {
